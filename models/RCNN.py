...
class RCNN(nn.Module):
	...
	def forward(self, input_sentence, batch_size=None):
		...
		# output, (final_hidden_state, final_cell_state) = self.lstm(input, (h_0, c_0))
		output, (final_hidden_state, final_cell_state) = self.lstm(input, self.init_hidden)
	...
	def initHidden(self, device):
		self.init_hidden = (torch.zeros(2, self.batch_size, self.hidden_size).to(device),
							torch.zeros(2, self.batch_size, self.hidden_size).to(device))